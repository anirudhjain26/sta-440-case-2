...
The distribution of raw JO₂ values was right-skewed, with a long tail toward higher flux (Figure @ref(fig-histograms)a). A log-transformation produced a more symmetric, approximately normal distribution (Figure @ref(fig-histograms)b), suggesting potential benefits for variance stabilization. However, since model assumptions were reasonably satisfied on the raw scale, the log transformation was used only for exploratory visualization and not in the final modeling.

Trajectories of JO₂ across ΔGATP levels revealed consistent dose–response relationships (Figure @ref(fig-jo2-dose-plots)a): respiration increased as workload rose, and transgenic (Tg) mice generally exhibited higher flux than non-transgenic (NT) controls. Group-level means with standard errors reinforced these patterns (Figure @ref(fig-jo2-dose-plots)b), showing the strongest Tg–NT separation for carbohydrate-linked substrates (GM, PM), moderate differences for mixed fuels (PMPc, PMOc), and smaller effects for fatty acid substrates (OcM, PcM).

Barplots comparing basal and low-dose conditions confirmed that Tg mitochondria already displayed higher respiration at early workloads (Figure @ref(fig-baseline-bar)). Boxplots of subject-specific JO₂–dose slopes highlighted that Tg mice exhibited steeper workload sensitivity, particularly under carbohydrate conditions (Figure @ref(fig-slopes-box)). Finally, a heatmap of mean JO₂ across substrates and doses illustrated the overall pattern of higher respiration in Tg mice, with the largest differences observed for carbohydrate fuels (Figure @ref(fig-heatmap-mean)).

Together, these exploratory findings highlighted the importance of genotype, substrate, and dose — and their interactions — as key factors for formal modeling. They also emphasized the need to account for repeated measures within subjects, since while overall patterns were consistent, baseline levels varied by mouse.
...
-   **Mixed-effects model (lmer(JO2 \~ Genotype \* Substrate \* Dose + (1 \| Subject)**
...
We reshaped the raw worksheet into a tidy long format, and restricted analysis to active respiration by removing Basal and the lowest dose (-12.95). We converted dose to a numeric ΔGATP scale for modeling and dropped missing values. Initial checks of distribution and scale (see Figure @fig-histograms-1 and @fig-histograms-2) informed the decision to fit models on raw JO2 while using log-transformed views only for EDA.

We then fit a progression of regression models to quantify effects and interactions: - Baseline linear model without genotype: `JO2 ~ Substrate * Dose` (for design checks). - Genotype-only model: `JO2 ~ Genotype` (coarse group difference). - Full linear model: `JO2 ~ Genotype * Substrate * Dose` (all interactions). - Mixed-effects model with subject intercepts: `JO2 ~ Genotype * Substrate * Dose + (1 | Subject)`.

For the linear models, we report coefficients and ANOVA tables (@tbl-lm-raw-coefs, @tbl-lm-raw-anova, @tbl-lm-multi-anova). For the mixed model, we summarize fixed effects (@tbl-lmm-fixed) and compare overall model fit across candidates using AIC/BIC and log-likelihood (@tbl-model-fit). Model adequacy was assessed via residual–fitted and normal Q–Q diagnostics (@fig-diag-plots-1 and @fig-diag-plots-2). To aid interpretation, we visualized fitted curves with 95% CIs from the mixed model (@fig-ggeffects-curves) and computed estimated marginal means and Tg–NT contrasts across Substrate × Dose cells (@fig-emm-heatmap; @tbl-emm-contrasts). Supporting exploratory figures show raw trajectories and group means over dose (@fig-jo2-dose-plots-1 and @fig-jo2-dose-plots-2), per-subject dose–response slopes (@fig-slopes-box), and mean JO2 heatmaps (@fig-heatmap-mean).

## Results

Across substrates, JO2 exhibits a clear dose-dependent response and substantial substrate-to-substrate variability (see trajectories and means in @fig-jo2-dose-plots-1 and @fig-jo2-dose-plots-2, and the mean heatmap in @fig-heatmap-mean). The baseline linear model confirms strong effects of Substrate and Dose and their interaction (@tbl-lm-raw-anova). Incorporating Genotype and its interactions in the full linear model indicates that genotype differences are not uniform; they depend on the substrate and/or dose context (@tbl-lm-multi-anova).

Accounting for repeated measures with a subject-level random intercept, the mixed-effects model provides the primary inference. Fixed-effect estimates in @tbl-lmm-fixed quantify the average genotype difference at the reference levels and how that difference varies with substrate and dose via interaction terms. The fitted curves from @fig-ggeffects-curves visualize these patterns: separation between Tg and NT varies by substrate and across ΔGATP, with uncertainty bands reflecting the modest sample size. The EMM contrast heatmap and table (@fig-emm-heatmap; @tbl-emm-contrasts) pinpoint specific Substrate × Dose combinations where Tg differs from NT and summarize effect sizes with standard errors and p-values.

Taken together, the results support dose- and substrate-dependent mitochondrial respiration with evidence of genotype effects that are context-specific. Where significant contrasts appear in @tbl-emm-contrasts, they align with the visual separation seen in @fig-ggeffects-curves, while diagnostics in @fig-diag-plots-1 and @fig-diag-plots-2 do not indicate major violations of linear model assumptions. Model comparison in @tbl-model-fit favors the mixed-effects specification, balancing improved fit with appropriate handling of within-subject correlation.
...
The study was also limited by sample size (6 non-transgenic mice and 6 transgenic mice) which reduces the statistical power, particularly for detecting complex interactions or subtle nonlinearities. To account for this, the analysis assumed a linear relationship between the dose and JO2 across the studied range. While the assumption appeared reasonable within the available data, nonlinear responses outside of the observed range cannot be ruled out. Finally, the mixed effects modeling approach included random intercepts to capture subject level variability but did not incorporate random slopes, potentially underestimating within subject differences in dose response patterns. However, residuals were approximately normal after accounting for fixed effects, and the assumption of independence across subjects was reasonable given the experimental design.

## Conclusions

Our analysis evaluated the effects of genotype, substrate and energetic workload on mitochondrial respiration in skeletal muscle using oxygen flux as the primary outcome. By applying a sequence of regression models and ultimately using a mixed effects model, we were able to account for both experimental design factors and within subject correlation to look into genotype dependent differences.

Our results found that strong dose and substrate dependent variation in mitochondrial respiration. These results suggest that the transgenic modification influences mitochondrial efficiency in a manner that depends on the energetic environment, rather than exerting a uniform effect across all conditions.

The mixed-effects model showed the most biologically and statistically relevant inference because it was able to balance complexity with the constraints of our small sample size. While there were significant contrasts that highlighted conditions where transgenic mitochondria differ from the non-transgenic controls, the broader interpretation should be take into account the limitations of the analysis, including small sample size, potential heteroscedasticity, and the restriction to oxygen flux as the only outcome we analyzed.

Overall, our analysis shows that looking at both substrate and dose together with genotype helps reveal patterns in how mitochondria function. The findings highlight the importance of considering experimental context when evaluating genetic influences on bioenergetics and provide a foundation for future studies that incorporate larger cohorts, have additional mitochondrial outcomes, and have more flexible modeling of the relationships between dose and response.
...
hist(
    log10(data_long$JO2),
    breaks = 60,
    main = "Distribution of Log Transformed JO2",
...
ggplot(heat, aes(x = Dose, y = Substrate, fill = mean_JO2)) +
    ...
    labs(title = "Heatmap of mean JO2 \
across Substrates & Doses") +
    theme(
...
lmm_coef <- as.data.frame(coef(summary(lmm)), stringsAsFactors = FALSE)
lmm_coef$Term <- rownames(lmm_coef)
lmm_coef <- dplyr::select(lmm_coef, Term, Estimate, `Std. Error`, df, `t value`, `Pr(>|t|)`)

# Safer header: avoid the vertical bars
cn <- c("Term", "Estimate", "Std. Error", "df", "t value", "p-value")

knitr::kable(lmm_coef, digits = 3, col.names = cn, booktabs = TRUE, escape = TRUE)
```
...
fit_tbl <- tibble::tibble(
    ...
    logLik = c(
        as.numeric(logLik(lm_marginal)), as.numeric(logLik(lm_raw)),
