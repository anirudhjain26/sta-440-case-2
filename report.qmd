...
doses <- c("-12.95", "-13.65", "-13.95", "-14.19", "-14.36", "-14.49")

tidy_data <- data.frame()
for (sub in names(substrates)) {
  cols <- substrates[[sub]]
  for (i in seq_along(cols)) {
    col_idx <- cols[i]
    tmp <- df[-1, c(1, col_idx)]  # drop header row, keep Subject + JO2 col
    colnames(tmp) <- c("Subject", "JO2")
    tmp$Substrate <- sub
    tmp$Dose <- doses[i]
...
# Convert factors
tidy_data$Substrate <- factor(tidy_data$Substrate)
tidy_data$Dose <- factor(tidy_data$Dose, levels = doses, ordered = TRUE)
tidy_data$Subject <- factor(tidy_data$Subject)

# ---- Fit raw-scale LM ----
lm_raw <- lm(JO2 ~ Substrate * Dose, data = tidy_data)
